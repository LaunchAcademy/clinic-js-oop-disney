self["NdbUi"]=self["NdbUi"]||{};Ndb.RunConfiguration=class extends UI.VBox{constructor(){super(true);this.registerRequiredCSS("ndb_ui/runConfiguration.css");this._items=new UI.ListModel;this._list=new UI.ListControl(this._items,this,UI.ListMode.NonViewport);this._npmExecPathPromise=null;this.contentElement.appendChild(this._list.element);this.update()}async update(){const configurations=[];const main=await Ndb.mainConfiguration();if(main)configurations.push(main);const pkg=await Ndb.backend.pkg();if(pkg){const scripts=pkg.scripts||{};this._items.replaceAll(configurations.concat(Object.keys(scripts).map(name=>({name,command:scripts[name],args:["run",name]}))))}}_npmExecPath(){if(!this._npmExecPathPromise)this._npmExecPathPromise=Ndb.backend.which("npm").then(result=>result.resolvedPath);return this._npmExecPathPromise}createElementForItem(item){const f=UI.Fragment.build`
    <div class=list-item>
    ...
    </div>`;const buttons=f.$("controls-buttons");const toolbar=new UI.Toolbar("",buttons);const runButton=new UI.ToolbarButton(Common.UIString("Run"),"largeicon-play");runButton.addEventListener(UI.ToolbarButton.Events.Click,this._runConfig.bind(this,item.execPath,item.args));toolbar.appendToolbarItem(runButton);const profileButton=new UI.ToolbarButton(Common.UIString("Start recording.."),"largeicon-start-recording");profileButton.addEventListener(UI.ToolbarButton.Events.Click,this._profileConfig.bind(this,item.execPath,item.args));toolbar.appendToolbarItem(profileButton);return f.element()}async _runConfig(execPath,args){await Ndb.nodeProcessManager.debug(execPath||await this._npmExecPath(),args)}async _profileConfig(execPath,args){await UI.viewManager.showView("timeline");await Common.console.showPromise();const action=UI.actionRegistry.action("timeline.toggle-recording");await action.execute();await this._runConfig(execPath||await this._npmExecPath(),args)}heightForItem(debuggerModel){return 12}isItemSelectable(debuggerModel){return false}selectedItemChanged(from,to,fromElement,toElement){}};UI.context.addFlavorChangeListener(SDK.DebuggerPausedDetails,_=>{const details=UI.context.flavor(SDK.DebuggerPausedDetails);if(!details)UI.context.setFlavor(SDK.DebuggerModel.CallFrame,null)});Ndb.NodeProcesses=class extends UI.VBox{constructor(){super(true);this.registerRequiredCSS("ndb_ui/nodeProcesses.css");const toolbar=new UI.Toolbar("process-toolbar",this.contentElement);this._pauseAtStartCheckbox=new UI.ToolbarSettingCheckbox(Common.moduleSetting("pauseAtStart"),Common.UIString("Pause at start"),Common.UIString("Pause at start"));this._pauseAtStartCheckbox.element.id="pause-at-start-checkbox";toolbar.appendToolbarItem(this._pauseAtStartCheckbox);this._emptyElement=this.contentElement.createChild("div","gray-info-message");this._emptyElement.id="no-running-nodes-msg";this._emptyElement.textContent=Common.UIString("No running nodes");this._treeOutline=new UI.TreeOutlineInShadow;this._treeOutline.registerRequiredCSS("ndb_ui/nodeProcesses.css");this.contentElement.appendChild(this._treeOutline.element);this._treeOutline.element.classList.add("hidden");this._targetToUI=new Map;SDK.targetManager.observeTargets(this)}async targetAdded(target){if(target.id()==="<root>")return;const processInfo=Ndb.nodeProcessManager.infoForTarget(target);if(!processInfo||processInfo.isRepl())return;const f=UI.Fragment.build`
